...
class MainMenu:
    ...
    def _pressed_select(self):
        ...
        if self.sel_index == SEL_START_GAME:
            ...
            self.game.add_fade(
                palette.FADE_STEP_TICKS_DEFAULT,
                palette.FADE_LEVEL_6,
                self.game.cycle_palette,
            )
        elif self.sel_index == SEL_EXIT_GAME:
            self.game.add_fade(
                palette.FADE_STEP_TICKS_DEFAULT, palette.FADE_LEVEL_0, pyxel.quit
            )
    ...
    def draw(self, shake_x, shake_y):
        ...
        if self.show_press_start:
            ...
            pyxel.blt(
                shake_x
                + SELECTIONS[self.sel_index][0]
                + SELECTIONS[self.sel_index][2]
                + 2,
                shake_y + SELECTIONS[self.sel_index][1],
                0,
                16,
        ...
        utils.draw_number_shadowed(
            shake_x + 68, shake_y + 20, globals.g_highscore, zeropad=6
        )  # highscore number
