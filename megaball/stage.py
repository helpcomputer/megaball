...
SLOPE_TILES = {
    ...
    utils.get_tile_index(80, 40): [
        45,
        constants.COLLIDE_BOTTOM_RIGHT,
    ],  # bottom-right 2
...
class PauseMenu:
    ...
    def _pressed_select(self):
        if self.sel_index == self.SEL_RESUME:
            ...
            self.stage.game.add_fade(
                5, palette.FADE_LEVEL_6, self.stage.game.cycle_palette
            )
    ...
    def update(self, last_inputs):
        ...
        if (
            input.BUTTON_START in last_inputs.pressed
            or input.BUTTON_A in last_inputs.pressed
        ):
    ...
    def draw(self, shake_x, shake_y):
        ...
        pyxel.blt(
            shake_x
            + self.SELECTIONS[self.sel_index][0]
            + self.SELECTIONS[self.sel_index][2]
            + 2,
            shake_y + self.SELECTIONS[self.sel_index][1],
            0,
            16,
...
class Stage:
    ...
    def player_death_anim_done(self):
        if globals.g_lives >= 1:
            globals.g_lives -= 1
            self.game.add_fade(
                palette.FADE_STEP_TICKS_DEFAULT,
                palette.FADE_LEVEL_6,
                self.game.restart_stage,
            )
        else:
    ...
    def _check_next_stage(self):
        ...
        self.game.add_fade(
            palette.FADE_STEP_TICKS_DEFAULT, palette.FADE_LEVEL_0, self.go_to_next_stage
        )
        # else:
        #    self.state = STATE_GAME_COMPLETE
    ...
    def go_to_next_stage(self):
        if self.next_stage_flash_num == 0:
            self.game.add_fade(
                palette.FADE_STEP_TICKS_SLOW,
                palette.FADE_LEVEL_3,
                self.go_to_next_stage,
            )
        elif self.next_stage_flash_num == 1:
            self.game.add_fade(
                palette.FADE_STEP_TICKS_SLOW,
                palette.FADE_LEVEL_0,
                self.go_to_next_stage,
            )
        ...
        #        palette.FADE_LEVEL_0, self.go_to_next_stage)
            ...
            if self.num == stage.MAX_STAGE_NUM:
                self.game.add_fade(
                    palette.FADE_STEP_TICKS_SLOW,
                    palette.FADE_LEVEL_6,
                    self.game.go_to_game_complete_stage,
                )
            else:
                globals.add_lives(1)
                self.game.add_fade(
    ...
    def update(self, last_inputs):
        if self.num > 0:  # dont allow inputs on demo/main menu stage 0.
            if self.pause_menu.is_visible:
                ...
                if (
                    input.BUTTON_START in last_inputs.pressed
                    or input.BUTTON_B in last_inputs.pressed
                ):
                    if self.state == STATE_PLAY or self.state == STATE_PLAYER_WEAPON:
                        self.pause_menu.is_visible = True
                else:
                    self.player.update(self, last_inputs)

                    if self.state == STATE_PLAY or self.state == STATE_DEMO:
